...
<body>
    <h1>Articles</h1>
    <p><a href="{{ route('admin.posts.create') }}">Create article</a> · <a href="{{ route('admin.dashboard') }}">Back to dashboard</a></p>

    @foreach ($posts as $post)
        <div class="card">
            <h3>{{ $post->title }}</h3>
            <p><strong>Category:</strong> {{ optional($post->category)->name ?? 'No category' }}</p>
            <p>{{ $post->excerpt }}</p>
            <p>
                ...
                <form method="POST" action="{{ route('admin.posts.destroy', $post) }}" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" style="border:0; background:none; color:#12553f; font-weight:700; cursor:pointer; padding:0;">Delete</button>
                </form>
            </p>
        </div>
    @endforeach
