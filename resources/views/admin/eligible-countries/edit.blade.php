...
<div class="admin-content">
    <div class="page-header">
        <div>
            <h1 class="page-title">{{ __('Edit Eligible Country') }}</h1>
            <p class="page-subtitle">{{ __('Update country information and settings') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.eligible-countries.update', $country) }}">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control" required value="{{ old('name', $country->name) }}">
                    @error('name')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="iso_code" class="form-label">{{ __('ISO Code (3 letters)') }}</label>
                    <input type="text" id="iso_code" name="iso_code" class="form-control" required maxlength="3" pattern="[A-Za-z]{3}" value="{{ old('iso_code', $country->iso_code) }}">
                    @error('iso_code')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="region" class="form-label">{{ __('Region') }}</label>
                    <input type="text" id="region" name="region" class="form-control" value="{{ old('region', $country->region) }}">
                    @error('region')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="display_color" class="form-label">{{ __('Display Color') }}</label>
                    <input type="color" id="display_color" name="display_color" class="form-control" value="{{ old('display_color', $country->display_color) }}">
                    @error('display_color')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="sort_order" class="form-label">{{ __('Sort Order') }}</label>
                    <input type="number" id="sort_order" name="sort_order" class="form-control" value="{{ old('sort_order', $country->sort_order) }}">
                ...
                <div class="form-group full-width">
                    ...
                    <textarea id="description" name="description" class="form-control" rows="3">{{ old('description', $country->description) }}</textarea>
                    @error('description')<div class="form-error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group full-width">
                    <label for="tooltip_text" class="form-label">{{ __('Tooltip Text') }}</label>
                    <textarea id="tooltip_text" name="tooltip_text" class="form-control" rows="2">{{ old('tooltip_text', $country->tooltip_text) }}</textarea>
